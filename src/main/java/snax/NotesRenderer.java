...
public class NotesRenderer {
    ...
    public static void renderNotesBox(GuiGraphics guiGraphics, int x, int y, int width, int height, 
        ...
        } else if (AIIntegration.isWaitingForResponse()) {
    ...
    private static int getLineColor(String line) {
        ...
        if (line.trim().startsWith("/clear")) {
            ...
            return 0xFF00FFFF; // Cyan for AI queries
        } else if (line.trim().startsWith(">")) {
            return 0xFF90EE90; // Light green for AI query display
        } else if (line.trim().equals("Thinking...")) {
            return 0xFFFFFF00; // Yellow for thinking indicator
        }
    ...
    private static void renderInstructionText(GuiGraphics guiGraphics, int x, int y, boolean isDragMode) {
        ...
        if (isDragMode) {
            ...
            guiGraphics.drawString(mc.font, "F7 settings", x + 3, y + 33, 0xFFFFFFFF);
            guiGraphics.drawString(mc.font, "Type /question for AI", x + 3, y + 43, 0xFF00FFFF);
            guiGraphics.drawString(mc.font, "Type /clear to reset", x + 3, y + 53, 0xFFFF6B6B);
        }
    }
