...
public class NetworkManager {
    ...
    public static void initNetwork() {
        if (!networkInitialized) {
            int packetId = 0;
            INSTANCE.registerMessage(packetId++, NotesUpdatePacket.class, NotesUpdatePacket::encode,
                    NotesUpdatePacket::decode, NotesUpdatePacket::handle);
            INSTANCE.registerMessage(packetId++, NotesRequestPacket.class, NotesRequestPacket::encode,
                    NotesRequestPacket::decode, NotesRequestPacket::handle);
                ...
                if (networkInitialized) {
                    try {
                        INSTANCE.send(PacketDistributor.PLAYER.with(() -> serverPlayer),
                                new NotesUpdatePacket(savedNotes));
                    } catch (Exception e) {
